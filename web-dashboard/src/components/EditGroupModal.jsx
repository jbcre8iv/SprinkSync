/**
 * SprinkSync - Edit Group Modal
 *
 * Modal dialog for editing existing zone groups
 */
...
const EditGroupModal = ({ isOpen, onClose, group, zones, onGroupUpdated }) => {
  ...
  useEffect(() => {
    if (group) {
      setFormData({
        ...
        default_duration: group.default_duration || 15,
        color: group.color || '#3B82F6',
        icon: group.icon || '🌱',
        custom_image: group.custom_image || null,
        zones: group.zones?.map(z => z.id) || []
      });
      setImagePreview(group.custom_image || null);
    }
  }, [group]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Group "${formData.name}" updated successfully!`);
      onGroupUpdated();
      onClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-900">Edit Zone Group</h2>
          <button
            onClick={onClose}
